...
export async function PATCH(request: NextRequest) {
    try {
        ...
        const { filename, sharedWith, visibility } = body;
        ...
        await updateGitHubFile(filename, updatedContent, `Update shared emails for ${filename}`);
        ...
        return NextResponse.json(
            { error: "Failed to update shared emails", details: error.message },
            { status: 500 }
        );
    }
}
